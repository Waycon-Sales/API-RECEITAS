...
async createTables(){    
   return conection.openDb().then(db=>{
        ...
        db.exec('CREATE TABLE IF NOT EXISTS categoria(id INTEGER PRIMARY KEY AUTOINCREMENT, categoria TEXT NOT NULL, descricao TEXT NOT NULL)');
        ...
        db.exec('CREATE TABLE IF NOT EXISTS favoritos(id_usuario INTEGER PRIMARY KEY, id_receita INTEGER PRIMARY KEY, FOREIGN KEY (id_usuario) REFERENCES usuario(id), FOREIGN KEY (id_receitas) REFERENCES receitas(id) )');
        db.exec('CREATE TABLE IF NOT EXISTS refeicao_planejada(id INTEGER PRIMARY KEY AUTOINCREMENT, horario TEXT NOT NULL, tipo TEXT NOT NULL, titulo TEXT, efetuada TEXT NOT NULL, id_usuario INTEGER NOT NULL, id_receita INTEGER, FOREIGN KEY (id_usuario) REFERENCES usuario(id), FOREIGN KEY (id_receitas) REFERENCES receitas(id))');
...
async dropTables(){
    return conection.openDb().then(db=>{
        ...
        db.exec('DROP TABLE IF EXISTS refeicoes');
